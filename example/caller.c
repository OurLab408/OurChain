...
int contract_main(int argc, char **argv)
{
	 ...
	 *	argv[4]:call callee back msg (for call callee)
	 */
	switch (atoi(argv[1])){
		...
		case 1:{
			...
			call_argv[1] = "1";	//goto which callee block
			call_argv[2] = argv[3]; //tell callee what callee id is
			call_argv[3] = argv[2]; //return to this contract
			call_argv[4] = "2";	//return to which block
			call_contract(argv[3], call_argc, call_argv);
		...
		case 3:{
			//test system cmd
    			err_printf("%s: case 3\n", argv[0]);
			//system("bitcoin-cli -regtest getinfo > ~/out &");
			int call_argc = 5;
			char* call_argv[call_argc];
			call_argv[1] = "1";	//goto which callee block
			call_argv[2] = argv[3]; //tell callee what callee id is
			call_argv[3] = argv[2]; //return to this contract
			call_argv[4] = "4";	//return to which block
		...
		case 4:{
    			err_printf("%s: case 4\n", argv[0]);
    			err_printf("%s: caller back\n", argv[0]);
    			err_printf("%s: msg from callee: %s\n", argv[0], argv[4]);
			break;
