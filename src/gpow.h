...
enum { REACH_n=-1, OUT_OF_NONCE=-2, OUT_OF_TRY=-3 };
    ...
    while (nMaxTries > 0) {
        ...
        if (CheckProofOfWork(hash)) {
            ...
            if (theQ.size() < m) { 
                ...
                AddHash(block.gpow, hash);
                if ((theQ.size() ==  m)) {
                    FirstSuccess = LastSuccess;
                    if (isConservativeGPoW()) break;
                    else
                        for (auto const& e : theQ) thePq.push(e); // move ??
                }
            }
            else {
